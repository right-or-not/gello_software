...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def _initialize_with_retries(self) -> bool:
        ...
        for attempt in range(self._max_retries):
            ...
            # 串口被占用时只报告并重试，绝不能用 fuser 杀死未知进程。
            if not self._check_port_availability():
                print(f"Port {self._port} is busy")
            ...
            try:
                ...
                # 失败发生在 openPort() 之后时必须立即释放文件描述符，
                # 否则下一轮重试会把当前进程误判为串口占用者。
    ...
    def _initialize_hardware(self):
        ...
        # GELLO 必须处于无扭矩示教状态。任一舵机没有响应都表示初始化失败，
        # 不能继续启动读取线程并误报 Successfully initialized。
                ...
                if dxl_comm_result != COMM_SUCCESS:
                    ...
                    if self._consecutive_read_failures >= 10:
                        self._read_error = RuntimeError(
                            "GELLO Dynamixel communication timed out 10 times "
                            f"consecutively (SDK result {dxl_comm_result})"
                        )
                    continue
    ...
    def get_joints(self) -> np.ndarray:
        ...
        # 首次读取失败时快速报错，避免调用方永远卡住；运行中连续超时也不能
        # 继续把旧角度当作实时 GELLO 输入。
        self._wait_for_first_read()
        self._raise_if_read_failed()
        assert self._joint_angles is not None
        _j = self._joint_angles.copy()
        return _j / 2048.0 * np.pi

    def _wait_for_first_read(self, timeout_s: float = 3.0) -> None:
        """等待第一帧完整反馈，超时或连续通信失败时抛出异常。"""
        ...
        while self._joint_angles is None or self._velocities is None:
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(
                    f"timed out waiting for GELLO feedback on {self._port}"
                )
            time.sleep(0.02)
    ...
    def _raise_if_read_failed(self) -> None:
        ...
        if self._read_error is not None:
            raise self._read_error
        if (
            self._last_successful_read is not None
            and time.monotonic() - self._last_successful_read > 1.0
        ):
            raise RuntimeError(
                f"GELLO feedback on {self._port} has been stale for more than 1.0s"
            )
