...
class MujocoRobotServer:
    def __init__(
        ...
        """
        Some robot models (currently YAM) include their gripper directly in
        the arm XML.  GELLO represents a gripper position in the normalized
        range [0, 1], while MuJoCo uses the actuator's native ctrlrange (for
        YAM this is [0, 0.041] metres).  Detect that actuator so commands and
        observations use the same units on both sides of the ZMQ interface.
        dm_control namespaces attached model elements (for example,
        ``yam_v0/gripper``), so match both the plain and namespaced names.
        """
        self._integrated_gripper_actuator_id = -1
        for actuator_id in range(self._model.nu):
            actuator_name = mujoco.mj_id2name(
                self._model, mujoco.mjtObj.mjOBJ_ACTUATOR, actuator_id
            )
            if actuator_name == "gripper" or (
                actuator_name is not None and actuator_name.endswith("/gripper")
            ):
                self._integrated_gripper_actuator_id = actuator_id
                break
        self._integrated_gripper_qpos_adr: Optional[int] = None
        if self._integrated_gripper_actuator_id >= 0:
            gripper_joint_id = int(
                self._model.actuator_trnid[
                    self._integrated_gripper_actuator_id, 0
                ]
            )
            self._integrated_gripper_qpos_adr = int(
                self._model.jnt_qposadr[gripper_joint_id]
            )
    ...
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        ...
        if self._integrated_gripper_actuator_id >= 0:
            ...
            gripper_min, gripper_max = self._model.actuator_ctrlrange[
                self._integrated_gripper_actuator_id
            ]
            normalized_gripper = np.clip(_joint_state[-1], 0.0, 1.0)
            _joint_state[-1] = gripper_min + normalized_gripper * (
    ...
    def get_observations(self) -> Dict[str, np.ndarray]:
        ...
        if self._integrated_gripper_actuator_id >= 0:
            assert self._integrated_gripper_qpos_adr is not None
            gripper_pos = self._data.qpos[self._integrated_gripper_qpos_adr]
            gripper_min, gripper_max = self._model.actuator_ctrlrange[
                self._integrated_gripper_actuator_id
            ]
            ...
            if gripper_range > 0:
                gripper_pos = np.clip(
                    (gripper_pos - gripper_min) / gripper_range, 0.0, 1.0
                )
    ...
    def serve(self) -> None:
        ...
        with mujoco.viewer.launch_passive(self._model, self._data) as viewer:
            while viewer.is_running():
                ...
                self._joint_state = self.get_observations()["joint_positions"]
